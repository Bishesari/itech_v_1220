<div class="text-center">
    <flux:tooltip content="حذف رشته" position="bottom">
        <flux:button icon="trash" size="xs" variant="primary" color="red" class="cursor-pointer"
                     x-on:click="$flux.modal('delete-field-{{ $field->id }}').show()">
        </flux:button>
    </flux:tooltip>


    <flux:modal name="delete-field-{{ $field->id }}" :show="$errors->isNotEmpty()" focusable class="md:w-96">
        <div class="space-y-6">
            <div>
                <flux:heading size="lg">{{__('حذف رشته ')}} <span
                        class="font-bold text-red-500 dark:text-red-400">{{ $field->title }}</span></flux:heading>
